...
class SimpleFireDetector:
    def __init__(self, root):
        ...
        self.selected_column = None  # 1-7 for columns
        self.selected_row = None     # 1-3 for rows
        self.selection_active = False
        self.selected_indices = []   # List of selected detection indices
        self.selection_mode = None   # "row" or "column" mode
    ...
    def setup_ui(self):
        ...
        # Create buttons 
        excel_btn = ttk.Button(control_frame1, text="Load Excel", command=self.load_excel)
        excel_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        video_btn = ttk.Button(control_frame1, text="Load Video", command=self.load_video)
        video_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        plot_btn = ttk.Button(control_frame1, text="Plot", command=self.run_detection)
        plot_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        resume_btn = ttk.Button(control_frame1, text="Resume from Annotations", command=self.resume_from_excel)
        resume_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        save_btn = ttk.Button(control_frame1, text="Save Results As...", command=self.save_results)
        ...
        search_btn = ttk.Button(search_controls, text="🔍 Search", command=self.search_detection)
        search_btn.pack(side=tk.LEFT, padx=(0, 20))
        
        # Navigation buttons
        first_btn = ttk.Button(search_controls, text="⏮️ First", command=self.go_to_first_batch, width=8)
        first_btn.pack(side=tk.LEFT, padx=(0, 5))
        
        last_btn = ttk.Button(search_controls, text="⏭️ Last", command=self.go_to_last_batch, width=8)
        ...
        # Selection info
        selection_frame = ttk.LabelFrame(right_frame, text="Selection Info")
        selection_frame.pack(fill=tk.X, pady=(0, 10))
        
        self.selection_label = ttk.Label(selection_frame, text="No selection", wraplength=180)
        ...
        ttk.Label(legend_frame, text="🔵 Blue: True (Scroll Wheel / A)", foreground="blue").pack(anchor=tk.W)
        ttk.Label(legend_frame, text="🔴 Red: False (Left Click / D)", foreground="red").pack(anchor=tk.W)
        ttk.Label(legend_frame, text="🟢 Green: Unclear (Right Click / S)", foreground="green").pack(anchor=tk.W)
        ttk.Label(legend_frame, text="⌨️ → Arrow: Next Batch", foreground="purple", font=("Arial", 9, "bold")).pack(anchor=tk.W)
        ttk.Label(legend_frame, text="← Arrow: Prev Batch", foreground="orange", font=("Arial", 9, "bold")).pack(anchor=tk.W)
        ...
        ttk.Label(legend_frame, text="Ctrl+A: Select All", foreground="darkgreen", font=("Arial", 9, "bold")).pack(anchor=tk.W)
        ttk.Label(legend_frame, text="R+number: Row, C+number: Column", foreground="darkred", font=("Arial", 9, "bold")).pack(anchor=tk.W)
        ttk.Label(legend_frame, text="Esc: Deselect", foreground="gray", font=("Arial", 9, "bold")).pack(anchor=tk.W)
        ttk.Label(legend_frame, text="🚫 Spacebar: Disabled", foreground="red", font=("Arial", 8)).pack(anchor=tk.W)
    
    def setup_key_bindings(self):
        """Setup all key bindings"""
        self.root.focus_set()
        
        # PROPER SOLUTION: Unbind spacebar from ALL buttons globally
        # Note: ttk.Button uses "TButton" class, not "Button"
        self.root.unbind_class("Button", "<Key-space>")      # For tk.Button
        self.root.unbind_class("TButton", "<Key-space>")     # For ttk.Button
        
        # Make root focusable and bind keys
        self.root.bind("<Key>", self.on_key_press)
        self.root.bind("<Control-a>", self.select_all)
        self.root.bind("<Control-A>", self.select_all)
        self.root.bind("<Escape>", self.deselect_all)
        self.root.bind("<Left>", self.prev_batch_key)
        self.root.bind("<Right>", self.next_batch_key)
        
        # R + number for rows, C + number for columns
        self.root.bind("<Key-r>", self.start_row_selection)
        self.root.bind("<Key-R>", self.start_row_selection)
        self.root.bind("<Key-c>", self.start_column_selection)
        self.root.bind("<Key-C>", self.start_column_selection)
        
        # A,S,D for annotations
        self.root.bind("<Key-a>", self.annotate_blue)
        self.root.bind("<Key-A>", self.annotate_blue)
        self.root.bind("<Key-s>", self.annotate_green)
        self.root.bind("<Key-S>", self.annotate_green)
        self.root.bind("<Key-d>", self.annotate_red)
        self.root.bind("<Key-D>", self.annotate_red)
    
    
    def start_row_selection(self, event):
        """Start row selection mode - next number will be row"""
        self.selection_mode = "row"
        self.log_status("📍 Row selection mode - press 1-3 for row number")
        return "break"

    def start_column_selection(self, event):
        """Start column selection mode - next number will be column"""
        self.selection_mode = "column"
        self.log_status("📍 Column selection mode - press 1-7 for column number")
        return "break"
    
    def handle_number_key(self, number):
        """Handle number key press - check selection mode"""
        if self.selection_mode == "row":
            self.select_row(number)  # Directly select the row
        elif self.selection_mode == "column":
            self.select_column(number)  # Directly select the column
        else:
            # If no selection mode is active, set as column selection by default
            self.select_column(number)
    
    def on_key_press(self, event):
        """Handle key press events"""
        # Don't process keys when search entry has focus
        ...
        if event.state & 0x4:  # Ctrl key pressed
            ...
            # Handle number based on current selection mode
            if self.selection_mode == "row":
                if 1 <= number <= 3:
                    self.select_row(number)
                    self.selection_mode = None  # Reset after selection
                else:
                    self.log_status(f"⚠️ Row {number} invalid - use 1-3")
            elif self.selection_mode == "column":
                if 1 <= number <= 7:
                    self.select_column(number)
                    self.selection_mode = None  # Reset after selection
                else:
                    self.log_status(f"⚠️ Column {number} invalid - use 1-7")
            else:
                # No selection mode active - default behavior (column selection)
                if 1 <= number <= 7:
                    self.select_column(number)
                else:
                    self.log_status(f"⚠️ Default column selection - number {number} invalid (use 1-7)")
                    
        elif key in ['r', 'c']:
            if key == 'r':
                self.start_row_selection(event)
            elif key == 'c':
                self.start_column_selection(event)
        elif key in ['a', 's', 'd']:
            if key == 'a':
                self.annotate_blue(event)
            elif key == 's':
                self.annotate_green(event)
            elif key == 'd':
                self.annotate_red(event)
        elif key == 'left':
            self.prev_batch_key(event)
        elif key == 'right':
            self.next_batch_key(event)

    def select_all(self, event):
        """Select all detections in current batch"""
        ...
        self.log_status(f"📋 Selected all {len(self.selected_indices)} detections in current batch")
        return "break"
    
    def select_column(self, col):
        """Select entire column (1-7)"""
        if not self.detection_boxes:
            return
        
        self.selected_column = col
        self.selected_row = None  # Clear row selection when selecting column
        self.update_selection_based_on_grid()
        self.log_status(f"📍 Selected entire column {col}")

    def select_row(self, row):
        """Select entire row (1-3)"""
        if not self.detection_boxes:
            return
        
        self.selected_row = row
        self.selected_column = None  # Clear column selection when selecting row
        self.update_selection_based_on_grid()
        self.log_status(f"📍 Selected entire row {row}")
    
    def update_selection_based_on_grid(self):
        """Update selection based on column and row"""
        if not self.detection_boxes:
            return
        ...
        # Calculate grid layout (same as update_display)
        cols = min(len(current_batch), 7)  # Max 7 columns
        ...
        if self.selected_column is not None:
            ...
            for i in range(len(current_batch)):
                if (i % cols) == (self.selected_column - 1):
                    self.selected_indices.append(start_idx + i)
                    
        elif self.selected_row is not None:
            # Select entire row
            row_start = (self.selected_row - 1) * cols
            row_end = min(row_start + cols, len(current_batch))
            for i in range(row_start, row_end):
                if i < len(current_batch):
                    self.selected_indices.append(start_idx + i)
        
        self.selection_active = len(self.selected_indices) > 0
        self.update_selection_display()
    ...
    def deselect_all(self, event):
        ...
        self.selected_column = None
        self.selected_row = None
        self.selection_mode = None  # Reset selection mode
        self.update_selection_display()
        self.log_status("❌ Deselected all")
        return "break"
    
    def update_selection_display(self):
        """Update the selection info display"""
        if not self.selection_active or not self.selected_indices:
            self.selection_label.config(text="No selection")
            self.update_display()  # Refresh to remove highlights
            return
        
        if self.selected_column is not None:
            text = f"Selected: Column {self.selected_column}\n{len(self.selected_indices)} detections"
        elif self.selected_row is not None:
            text = f"Selected: Row {self.selected_row}\n{len(self.selected_indices)} detections"
        elif len(self.selected_indices) == 1:
            idx = self.selected_indices[0]
            detection = self.detection_boxes[idx]
            text = f"Selected: Detection {idx + 1}\nType: {detection['type']}\nConf: {detection['confidence']:.1%}"
        else:
            text = f"Selected: {len(self.selected_indices)} detections"
        
        self.selection_label.config(text=text)
        self.update_display()  # Refresh to show highlights
    
    def annotate_blue(self, event):
        """Annotate selected detections as True (Blue)"""
        self.annotate_selected(True, "Blue (True)")
        return "break"
    
    def annotate_green(self, event):
        """Annotate selected detections as Unclear (Green)"""
        self.annotate_selected("unclear", "Green (Unclear)")
        return "break"
    
    def annotate_red(self, event):
        """Annotate selected detections as False (Red)"""
        self.annotate_selected(False, "Red (False)")
        return "break"
    
    def annotate_selected(self, annotation_value, description):
        """Apply annotation to all selected detections"""
        if not self.selection_active or not self.selected_indices:
            self.log_status("⚠️ No detections selected")
            return
        
        count = 0
        for idx in self.selected_indices:
            if idx < len(self.detection_boxes):
                self.detection_boxes[idx]['annotated'] = annotation_value
                count += 1
        
        self.update_display()
        self.log_status(f"⌨️ Annotated {count} detections as {description}")
    
    def prev_batch_key(self, event):
        """Handle left arrow key for previous batch"""
        if self.search_entry != self.root.focus_get():
            self.prev_batch()
        return "break"
    
    def next_batch_key(self, event):
    ...
    def update_display(self):
        ...
        max_cols = 7  # Changed from 8 to 7 to match keyboard selection
        ...
        thumb_size = max(thumb_size, 70)  # Reduced minimum size from 80 to 70
        thumb_size = min(thumb_size, 130)  # Reduced maximum size from 150 to 130
        ...
        for i, detection in enumerate(current_batch):
            try:
                ...
                # Determine border color based on annotation status
                ...
                # Check if this detection is selected and add selection highlight
                if actual_idx in self.selected_indices:
                    # Add selection border (yellow outer border)
                    selection_offset = border_width + 5
                    self.canvas.create_rectangle(
                        x_pos - thumb_size//2 - selection_offset, 
                        y_pos - thumb_size//2 - selection_offset,
                        x_pos + thumb_size//2 + selection_offset, 
                        y_pos + thumb_size//2 + selection_offset,
                        outline="yellow", width=4, tags=f"thumb_{actual_idx}"
                    )
    ...
    def next_batch(self):
        ...
        # Clear selection when moving to new batch
...
def main():
    print("Starting Enhanced Fire Detection App with Keyboard Controls...")
